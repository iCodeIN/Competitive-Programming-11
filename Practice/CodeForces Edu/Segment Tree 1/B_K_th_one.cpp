...
private:
    ...
    {
        //
        return {a.c + b.c, max(a.idx, b.idx)};
    }
    ...
    item query(int x, int lx, int rx, int k)
    {

        // cout << lx << " " << rx << " " << tree[x].c << " " << k << endl;
        if (tree[x].c < k)
            return neutralValue;
        ...
        item ans1 = tree[(x << 1) + 1];
        item ans2 = tree[(x << 1) + 2];

        if (ans1.c >= k)
            return query((x << 1) + 1, lx, mx, k);
        else
            return query((x << 1) + 2, mx, rx, k - ans1.c);
    }
